...
#         for di,dj in ((-1,0),(1,0),(0,-1),(0,1)):
#             ni,nj = ci+di, cj+dj
#             if 0<=ni<N and 0<=nj<N and unknown[ni][nj]==0 and v[ci][cj]+1<v[ni][nj]:
#                 q.append((ni,nj))
#                 v[ni][nj]=v[ci][cj]+1
...
#     for wi,wj,wd,wv in anomaly:        # wv 단위로 wd방향으로 확산표시(출구가 아닌경우만 확산)
#         v[wi][wj]=1
#         for mul in range(1, N+1):
#             wi,wj = wi+di[wd], wj+dj[wd]
#             if 0<=wi<N and 0<=wj<N and unknown[wi][wj]==0 and (wi,wj)!=(u_ei,u_ej):
#                 if v[wi][wj]>wv*mul:    # 더 큰 값 일때만 갱신(겹칠수있으니)
#                     v[wi][wj]=wv*mul
#             else:
#                 break
#         # [4] 2차원 시작위치에서 BFS로 탈출구탐색(v에 적혀있는 값보다 작은 경우 지나감)
#         dist = bfs_2d(v, dist, u_si, u_sj, u_ei, u_ej)
